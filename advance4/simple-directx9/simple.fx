...
texture g_WeakBloomTex;
sampler WeakBloomSampler = sampler_state
{
    Texture = <g_WeakBloomTex>;
    MinFilter = LINEAR;
    MagFilter = LINEAR;
...
{
    float4 sceneColor = tex2D(SrcSampler, uv);
    float4 bloomColor = lerp(tex2D(WeakBloomSampler, uv), tex2D(BloomSampler, uv), g_BloomBlend);

    return sceneColor + bloomColor;
}

float4 PS_BloomThreshold(float2 uv : TEXCOORD0) : COLOR
{
    float4 sceneColor = tex2D(SrcSampler, uv);
    float mask = (Luminance(sceneColor.rgb) >= g_BloomThreshold) ? 1.0f : 0.0f;
...
technique BloomComposite
{
    pass P0
    {
        PixelShader = compile ps_3_0 PS_BloomComposite();
    }
}
...
technique BloomThreshold
{
    pass P0
    {
        PixelShader = compile ps_3_0 PS_BloomThreshold();
    }
}
