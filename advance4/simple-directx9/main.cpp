...
LPDIRECT3DTEXTURE9 g_pWeakBloomTex = NULL;
...
static void DrawThresholdCurrentRT(LPDIRECT3DTEXTURE9 sourceTex);
static void DrawBloomCurrentRT(LPDIRECT3DTEXTURE9 sceneTex, LPDIRECT3DTEXTURE9 strongBloomTex, LPDIRECT3DTEXTURE9 weakBloomTex, float blend);
...
void Render()
{
    ...
    // Bloom 用の二値マスクを作る。閾値以上は白、未満は黒。
    {
        IDirect3DSurface9* thresholdRT = NULL;
        g_pTempTex->GetSurfaceLevel(0, &thresholdRT);
        g_pd3dDevice->SetRenderTarget(0, thresholdRT);
        SAFE_RELEASE(thresholdRT);

        g_pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET, 0, 1.0f, 0);
        g_pd3dDevice->BeginScene();
        DrawThresholdCurrentRT(g_pSceneTex);
    ...
    // Down チェーン
    {
        ...
        LPDIRECT3DTEXTURE9 sourceTex = g_pTempTex;
    ...
    if (g_activeBlurLevels > 0 && g_compositeBlend < 1.0f)
    {
        ...
        if (g_activeBlurLevels > 1)
        {
            ...
            g_pWeakBloomTex->GetSurfaceLevel(0, &weakRT);
    ...
    // 最終出力
    {
        ...
        {
            g_effectFilterSpacing = 1.0f;
            DrawBloomCurrentRT(g_pSceneTex,
                               (g_compositeBlend < 1.0f) ? g_pTempTex : g_texUp[0],
                               (g_compositeBlend < 1.0f) ? g_pWeakBloomTex : g_texUp[0],
                               g_compositeBlend);
        }
        DrawOverlayText();
        g_pd3dDevice->EndScene();
...
void DrawThresholdCurrentRT(LPDIRECT3DTEXTURE9 sourceTex)
{
    IDirect3DSurface9* pRT = NULL;
    g_pd3dDevice->GetRenderTarget(0, &pRT);
    D3DSURFACE_DESC rtDesc = {};
    ...
    g_pEffect->SetTechnique("BloomThreshold");
    g_pEffect->SetTexture("g_SrcTex", sourceTex);
    g_pEffect->SetFloat("g_BloomThreshold", g_bloomThreshold);
...
void DrawBloomCurrentRT(LPDIRECT3DTEXTURE9 sceneTex, LPDIRECT3DTEXTURE9 strongBloomTex, LPDIRECT3DTEXTURE9 weakBloomTex, float blend)
{
    IDirect3DSurface9* pRT = NULL;
    g_pd3dDevice->GetRenderTarget(0, &pRT);
    D3DSURFACE_DESC rtDesc = {};
    ...
    g_pEffect->SetTechnique("BloomComposite");
    g_pEffect->SetTexture("g_SrcTex", sceneTex);
    g_pEffect->SetTexture("g_BloomTex", strongBloomTex);
    g_pEffect->SetTexture("g_WeakBloomTex", weakBloomTex);
    g_pEffect->SetFloat("g_BloomBlend", blend);
    g_pEffect->SetFloat("g_BloomThreshold", g_bloomThreshold);
...
void UpdateInput()
{
    ...
    const bool key9 = (GetAsyncKeyState('9') & 0x8000) != 0;
    const bool key0 = (GetAsyncKeyState('0') & 0x8000) != 0;
    ...
    if (g_bloomThreshold < 0.0f) { g_bloomThreshold = 0.0f; }
    if (g_bloomThreshold > 1.0f) { g_bloomThreshold = 1.0f; }
